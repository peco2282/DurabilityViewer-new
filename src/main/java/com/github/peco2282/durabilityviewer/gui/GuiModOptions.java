  ...
  protected void init() {
    ...
    for (String option : options) {
      ...
      if (value == null) {
        ...
        element = this.addRenderableWidget(new AbstractWidget(this.width / 2 + 10, y, buttonWidth, BUTTONHEIGHT, (Boolean) value ? trueText : falseText) {
          @Override
          public void onClick(double x, double y) {
        ...
        element = this.addRenderableWidget(new EditBox(this.getFontRenderer(), this.width / 2 + 10, y, buttonWidth, BUTTONHEIGHT, Component.literal((String) value)) {
          ...
          public boolean charTyped(char chr, int keyCode) {
            ...
            handler.onConfigChanging(new ConfigChangedEvent.OnConfigChangingEvent(modName, option, this.getValue()));
            return result;
          }
        ...
        element = this.addRenderableWidget(new AbstractWidget(this.width / 2 + 10, y, buttonWidth, BUTTONHEIGHT, Component.empty()) {
          ...
          @Override
          public boolean changeFocus(boolean ignored) {
            setMessage(null);
            return ignored;
          }
    ...
    if (colorSelector.visible) {
      ...
      for (int i = 0; i < this.options.size(); i++) {
        if (y > TOP_BAR_SIZE - LINEHEIGHT / 2 && y < height - BOTTOM_BAR_SIZE) {
          font.draw(stack, Component.translatable(options.get(i)).getString(), this.width / 2f - 155, y + 4, 0xffffff);
          ((AbstractWidget) this.children().get(i * 2 + 1)).setY(y);                                          // config elem
          ((AbstractWidget) this.children().get(i * 2 + 1)).render(stack, mouseX, mouseY, partialTicks);
          ((AbstractWidget) this.children().get(i * 2 + 2)).setY(y);                                        // reset button
